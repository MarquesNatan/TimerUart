...
static uint64_t global_timer_value = 0x00;
...
typedef struct {
    ...
    void Timer0_SetTickHook(void (*tickFunc)(uint64_t*));

    void tickHook_Execute(uint64_t* global_timer_value);

    uint64_t Timer0_GetGlobalTime( void );
...
# 1 "src/board/peripheral_controller/peripheral_controller.h" 1
# 11 "src/board/peripheral_controller/peripheral_controller.h"
    void Peripheral_Controller(void* args);
# 8 "src/main.c" 2

# 1 "src/board/board_definitions/board_definitions.h" 1
# 9 "src/main.c" 2
...
extern uint64_t global_timer_value;

uint64_t last_time_blink_led_0 = 0;
uint64_t last_time_blink_led_1 = 0;
uint8_t serial_byte_received = 0;

void tickHook_func(uint64_t *timer_value)
{
    if((*timer_value - last_time_blink_led_0) >= 5)
    {
        LATB = (PORTB ^ (1 << 0));;
        last_time_blink_led_0 = *timer_value;
    }
    if((*timer_value - last_time_blink_led_1) >= 10)
    {
        LATB = (PORTB ^ (1 << 1));;
...
void __attribute__((picinterrupt(("")))) TASK_Manager(void)
{
    ...
    if(PIR1bits.RCIF)
    {
        ...
        if(serial_byte_received == 0x41)
        {
            if(0x01 == 0x01) LATB = (PORTB | (1 << 2)); else LATB = (PORTB & ~((1 << 2)));;
        }else if(serial_byte_received == 0x44)
        {
